...
define('AUTH_HOST', 'ldap://ldap');
define('AUTH_PORT', 389);
define('AUTH_BIND_DN', 'cn=testrail,ou=authusers,dc=example,dc=com');
define('AUTH_BIND_PASSWORD', 'qwerty12345');
define('AUTH_DN', 'ou=users,dc=example,dc=com');
define('AUTH_FILTER', '(&(uid=%name%)(objectClass=posixAccount))');
define('AUTH_FALLBACK', true);
define('AUTH_CREATE_ACCOUNT', true);
define('AUTH_NAME_ATTRIBUTE', 'cn');
...
function authenticate_user($name, $password)
{
	...
	#file_put_contents('/tmp/debug.txt', cleanUpEntry($ldap_user) , FILE_APPEND | LOCK_EX);	
	...
	#file_put_contents('/tmp/debug.txt', $dn , FILE_APPEND | LOCK_EX);         
	#file_put_contents('/tmp/debug.txt', "\n\r" , FILE_APPEND | LOCK_EX);
	try
	{
	...
	// Code section for authenticate user using testrail-users LDAP group $group_name
	$group_name = "testrail-users";
	$dn2 = "ou=groups,dc=example,dc=com"; // Location of groups in directory
	$filter2 = sprintf("(cn=%s)", $group_name);
	...
	$search2 = ldap_search($handle, $dn2, $filter2);
	$results2 = ldap_get_entries($handle, $search2);
	_ldap_close_connection($handle);
	#file_put_contents('/tmp/debug.txt', $results2[0]['memberuid']['count'], FILE_APPEND | LOCK_EX);
	#file_put_contents('/tmp/debug.txt', $results2['count'] , FILE_APPEND | LOCK_EX);	
	#file_put_contents('/tmp/debug.txt', "\n\r" , FILE_APPEND | LOCK_EX);
	#file_put_contents('/tmp/debug.txt', $results2[1][0] , FILE_APPEND | LOCK_EX);
    #file_put_contents('/tmp/debug.txt', "\n\r" , FILE_APPEND | LOCK_EX);

	$ALLOWED_USER = false;
	
	#file_put_contents('/tmp/debug.txt', $ALLOWED_USER ? 'true' : 'false' , FILE_APPEND | LOCK_EX);	
	#file_put_contents('/tmp/debug.txt', "\n\r" , FILE_APPEND | LOCK_EX);

	for ($item = 0; $item < $results2['count']; $item++)
		{
	  		for ($attribute = 0; $attribute < $results2[$item]['count']; $attribute++)
	  			{
	   				$data = $results2[$item][$attribute];
	   				#echo $data.":&nbsp;&nbsp;".$results2[$item][$data][0]."<br>";
	   				for ($login=0; $login < $results2[$item][$data]['count'] ; $login++) { 
		   				if ($name == $results2[$item][$data][$login]) {
			 			$ALLOWED_USER = true;
						}
						#file_put_contents('/tmp/debug.txt', $results2[$item][$data][$login], FILE_APPEND | LOCK_EX);	
						#file_put_contents('/tmp/debug.txt', "\n\r" , FILE_APPEND | LOCK_EX);
	   				}
	  			}
	}
	...
	if (!$ALLOWED_USER) {
		throw new AuthException(
			'User is not allowed to use testrail. Please, contact testrail amdin'
		);
	}
...
$file = '/tmp/debug.txt';
#file_put_contents($file, $handle, FILE_APPEND | LOCK_EX);	
try
	{
